    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <!-- table -->
                @include('livewire.category.edit-categories')
                <div class="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
                    <div class="px-6 py-4 flex items-center">
                        ...
                        <x-input type="text" class="flex-1 mx-4" placeholder="Busca: Nombre" wire:model="search" />
                        @livewire('category.create-categories')
                    </div>
                    @if (count($categories))
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" wire:click="order('slug')"
                                        class="cursor-pointer px-6 py-4 font-medium text-gray-900">Slug
                                        {{-- Sort de la columna --}}
                                        @if ($sort == 'slug')
                                            @if ($direction == 'asc')
                                                <i class="fa-solid fa-arrow-up-a-z float-right mt-1"></i>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            <a x-data="{ tooltip: 'Edite' }" href="#" wire:click="edit({{ $item}}, '{{ $item->name }}')">
                                                ...
                                                <a x-data="{ tooltip: 'Delete' }" href="#"  wire:click="$emit('evtDeleteCategory', {{$item->id}})">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                        class="h-6 w-6" x-tooltip="tooltip">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
...
@push('scripts')
        <script>
            ...
            Livewire.on('evtDeleteCategory', id => {
                Swal.fire({
                    title: '¿Estas seguro?',
